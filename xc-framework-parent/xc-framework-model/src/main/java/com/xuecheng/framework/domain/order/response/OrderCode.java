...
public enum OrderCode implements ResultCode {
    ORDER_ADD_ITEMISNULL(false,400,"创建订单未选择课程！"),
    ORDER_ADD_ORDERNUMERROR(false,4002,"生成订单号错误！"),
    ORDER_ADD_GETCOURSEERROR(false,4003,"找不到课程信息！"),
    ORDER_FINISH_NOTFOUNDORDER(false,4004,"找不到订单信息！"),
    Pay_NOTFOUNDORDER(false,400,"找不到要支付的订单！"),
    Pay_USERERROR(false,40,"支付用户与订单用户不一致！"),
    Pay_NOTFOUNDPAY(false,402,"支付记录不存在！");
    ...
    //操作代码
    @ApiModelProperty(value = "媒资系统操作代码", example = "2200", required = true)
    int code;
    //提示信息
